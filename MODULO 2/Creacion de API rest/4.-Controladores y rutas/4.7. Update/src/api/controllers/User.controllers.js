...
/**
 * *PASOS A SEGUIR:
 * 1) Tener en cuenta que cosas podemos y no actualizar:
 * - rol
 * - _id
 * - email
 * - password
 ...
 * 2) body: name, image
 *
 * MUCHO CUIDADO, CUANDO QUIERO HACER ALGO PARA ACTUALIZAR EL GENERO, HAY QUE COMPROBAR
 * DE FORMA MANUAL QUE ESTÉ DENTRO DEL ENUM (en models -user.model)
 *
 * 3) tenemos un token --> _Id lo sacamos gracias a la req.user
 * 4) findByAndUpdate
 * 5) test para ver si se ha actualizado
 *
 *
 * ERRORES:
 *  - no recibir el token, lo vamos a hechar por el middleware de auth (autentificación)
 *
 *   */
...
const update = async (req, res, next) => {
  ...
  try {
    ...
    const patchUser = new User(req.body); // nuevo usuario parqueado con la req.body que he recibido
    ...
    try {
      ...
      // ++++++++++++++++++++++ TEST RUNTIME+++++++++++++++++++++++++++++++++++++++
      // RUNTIME = tiempo real de ejeucion de la aplicación
      /** siempre lo primero cuando testeamos es el elemento actualizado para comparar la info que viene
       * del req.body
       */
      const updateUser = await User.findById(req.user._id);

      /** sacamos las claves del objeto del req.body (con el object.keys) para saber que info nos han pedido actualizar */
      ...
      ///----------------> para todo lo diferente de la imagen que nos pidan actualizar------------------------------
      updateKeys.forEach((item) => {
        // para recorrer cada una de las claves que nos piden actualizar
        ...
        if (updateUser[item] === req.body[item]) {
          ...
          if (updateUser[item] != req.user[item]) {
            // req.user = el usuario que había antes de actualizarse
            // si es diferente a lo que ya teniamos lanzamos el nombre de la clave y su valor como true en un objeto
            // este objeto se pusea en el array que creamos arriba que guarda todos los testing en el runtime
            testUpdate.push({
              [item]: true,
            });
          } else {
            // si son igual lo que pusearemos sera el mismo objeto que arrriba pero diciendo que la info es igual
            testUpdate.push({
              [item]: "sameOldInfo",
            });
          }
