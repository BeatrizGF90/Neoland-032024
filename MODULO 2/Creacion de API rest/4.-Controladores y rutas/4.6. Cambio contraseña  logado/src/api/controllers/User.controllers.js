...
const modifyPassword = async (req, res, next) => {
  ...
  /** IMPORTANTE ---> REQ.USER ----> LO CREAMOS EN LOS AUTH MIDDLEWARE */
  ...
  try {
    ...
    /** comparamos la contrasela vieja sin encriptar y la encriptada req.user.password
     * que es la creamos en la AUTH MIDDLEWARE*/
    if (bcrypt.compareSync(password, req.user.password)) {
      /** si es correcto, tenemos que encriptar la contraseña (con 10 vueltas) para
       * poder guardarla en el back mongo db */
      const newPasswordHashed = bcrypt.hashSync(newPassword, 10);

      /** vamos a actualizar la contraseña en mongo db */
      // SIMPRE QUE HAY UN BORRADO U ACTUALIZACION NUEVA HAY UN TRY CATCH
      try {
        await User.findByIdAndUpdate(_id, { password: newPasswordHashed });
        /* Al usuario que encontremos con el id le vamos a meter en la contraseña 
        la nueva nueva contraseña encriptada */
