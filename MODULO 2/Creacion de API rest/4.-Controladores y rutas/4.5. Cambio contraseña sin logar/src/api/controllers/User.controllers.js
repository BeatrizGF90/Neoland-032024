...
const changePassword = async (req, res, next) => {
  ...
  try {
    ...
    if (userDb) {
      ...
      return res.redirect(
        307,
        `http://localhost:${PORT}/api/v1/users/sendPassword/${userDb._id}` //Llamamos a un controlador que se encarga de enviar la contraseña
      );
    } else {
      return res.status(404).json("User no register");
    }
...
const sendPassword = async (req, res, next) => {
  try {
    ...
    const { id } = req.params; // conseguimos el id
    const userDb = await User.findById(id); // buscamos el usuario en la base de datos con ese id
    const email = process.env.EMAIL; // traemos el email de ese usuario
    const password = process.env.PASSWORD; // traemos la password de ese usuario
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: email,
    ...
    let passwordSecure = randomPassword(); // generamos una contraseña random con la función randomPassword que está en utils
    ...
    const mailOptions = {
      ...
      // Muy importante no poner password de palabra porque el correo si ve password borra el email inmediatamente.
    };
    transporter.sendMail(mailOptions, async function (error, info) {
      if (error) {
        ...
        try {
           ...
           * en este caso le decimos que en la parte de password queremos meter
           * la contraseña hasheada (encriptada)
           */
          await User.findByIdAndUpdate(id, { password: newPasswordBcrypt });

          //!------------------ test --------------------------------------------
          //!-------------------Para comprobar que la contraseña está encriptada
          ...
          return res.status(404).json(error.message); // cuando es una respuesta hay que meter error.message
        }
      }
    });
  } catch (error) {
    return next(error); // si es un next solo hay que meter error, no hace falta error.message
  }
};
