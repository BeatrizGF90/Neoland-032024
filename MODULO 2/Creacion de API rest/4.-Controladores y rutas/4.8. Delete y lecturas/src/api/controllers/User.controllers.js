...
const deleteUser = async (req, res, next) => {
  try {
    const { _id, image } = req.user; // por el req.user hacemos un destructuring y cogemos el id y la imagen
    await User.findByIdAndDelete(_id); // hacemos la query findByIdAndDelete y le metemos el id del usuario

    // hacemos un test para ver si lo ha borrado
    if (await User.findById(_id)) {
      // si encontramos el id del usuario
      // si el usuario
      return res.status(404).json("not deleted"); /// retornamos un mensaje de que no ha sido borrado
    } else {
      // en el caso de que si lo ha borrado
      /**
       * HAY QUE BORRARR TODO LO QUE HAY HECHO EL USER: LIKE, COMENTARIOS, LOS CHATS, LOS POSTS , REVIEWS ....
       */
      deleteImgCloudinary(image); // borramos la imagen

      return res.status(200).json("ok delete"); // mandamos mensaje de usuario borrado
    }
  } catch (error) {
    return next(error);
  }
...
const byOnlyGender = async (req, res, next) => {
  try {
...
//! -----------------------------------------------------------------------------
//? ---------------------------------get By Gender and Name----------------------
//! -----------------------------------------------------------------------------

const byGender = async (req, res, next) => {
  try {
    const getGenderUser = await User.find({
      //si uno de los dos no existe, devuelve un array vacío
      gender: req.params.gender,
